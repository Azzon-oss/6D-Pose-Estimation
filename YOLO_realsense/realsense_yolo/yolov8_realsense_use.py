...
"""从 RealSense 相机中 获取一帧对齐后的彩色图 + 深度图 + 深度图对应的相机内参"""
def get_aligned_images():
    ...
    img_color = np.asanyarray(aligned_color_frame.get_data())  # 把彩色图转换成 NumPy 数组
    img_depth = np.asanyarray(aligned_depth_frame.get_data())  # 把深度图转换成 NumPy 数组
...
if __name__ == '__main__':
    model = YOLO(r'../recycle_best.pt')  # 加载权重文件，如需要更换为自己训练好的权重best.pt即可
    ...
    try:
        while True:
            ...
            # 检查是否达到间隔时间，如果已经过了 0.1 秒，就准备把当前图像送入 YOLO。
            if time.time() - start_time >= interval:
                ...
                for result in results:  # 相当于都预测完了才进行的打印目标框，这样就慢了
                    ...
                    for i in range(len(boxes)):  #遍历每个检测目标
                        ux, uy = int(boxes[i][0]), int(boxes[i][1])  # 提取检测框中心像素位置 (ux, uy)，也就是图像中目标中心点
                        dis = aligned_depth_frame.get_distance(ux, uy) #获取当前像素点对应的深度（Z 值）
                        camera_xyz = rs.rs2_deproject_pixel_to_point(
                            depth_intrin, (ux, uy), dis)  # 计算相机坐标系的xyz：需要像素坐标（u,v）;该点深度值 Z;相机内参（fx, fy, cx, cy）
